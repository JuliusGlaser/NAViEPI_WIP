...
# %% read in raw data
instr = DIR + '/' + args.data
outprefstr = '/home/hpc/iwbi/iwbi019h/DeepSubspaceMRI/deepsubspacemri/data_files/phantom_ms_qti/meas_MID00181_FID09427_ep2d_diff_ms_gufi_lin'
...
for s in slice_loop:
    ...
    if args.navi:
        # # muse
        print(navi_resi.shape)
        print(coil_resi.shape)
    ...
    if args.jets is True:
        ...
        for s in range(args.split):
            ...
            navi_shot_muse_phase_split = navi_shot_muse_phase[diff_idx,...]

            dwi_comb_jets = app.HighDimensionalRecon(
                                    kdat_prep_split, coil2,
                                    phase_sms=sms_phase,
                                    combine_echo=True,
                                    phase_echo=np.conj(navi_shot_muse_phase_split),
                                    regu='LLR',
                                    blk_shape=(1, 6, 6),
                                    blk_strides=(1, 1, 1),
                                    solver='ADMM',
            ...
            # store output
            f = h5py.File(outprefstr + '/JETS' + OUT_STR + split_str + '.h5', 'w')
            f.create_dataset('nav_shot_muse', data=navi_shot_muse_full)
